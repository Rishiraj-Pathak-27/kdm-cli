 ...
 * at the same time. Enforced by handleAssign in assign.cjs.
 * @type {number}
 */
 ...
 * redirected to Beginner and higher-level issues. Enforced by handleAssign in assign.cjs.
 * @type {number}
 */
...
function buildWelcomeComment(username, skillLevel) {
  ...
  if (isGoodFirstIssue) {
    return [
      `👋 Hi @${username}, welcome to the KDM CLI community! Thank you for choosing to contribute — we're thrilled to have you here! 🎉`,
      "",
  ...
  return [
    `👋 Hi @${username}, thanks for continuing to contribute to the KDM CLI! You've been assigned this **${skillDisplayName}** issue. 🙌`,
    "",
